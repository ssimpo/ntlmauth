...
var NTLMAuth = (function(config) { 
	...
	NTLMAuth.prototype.createSSPIRequest = function(domainName, clientName) {
		...
		ntlmPacket.writeString('NTLMSSP\0', 0, 8, 'utf8');
	...
	return ntlmPacket;
  	...
  	NTLMAuth.prototype.parseChallenge = function(challenge) {
		...
		var oddData = buffer.readBuffer(8);

		var domainName = buffer.readString(domainLen, "ucs2");
		var addData = buffer.readBuffer(addDataLen);
	...
	NTLMAuth.prototype.createResponse = function(domain, username, password) {
		...
		data = new WritableTrackingBuffer(packetLength);
		...
		data.writeBuffer(ntlmData);
		
		data.writeUInt32LE(0x0101);
		data.writeUInt32LE(0x0000);
		data.writeBuffer(this.createTimestamp(genTime));
		data.writeBuffer(this.client_nonce);
		data.writeUInt32LE(0x0000);
		data.writeBuffer(this.server_data);
		data.writeUInt32LE(0x0000);
	...
	NTLMAuth.prototype.createClientNonce = function() {
		...
		for ( var nidx=0; nidx<8; nidx++ ) {
			client_nonce.writeUInt8(nidx, Math.ceil(Math.random()*255))
		}
		if (this.debug) {
